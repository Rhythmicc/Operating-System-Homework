...
- 首先实现多线程模型并使用压测工具进行压测，对比多进程模型分析
- 针对两个模型处理socket请求、发送数据、读取网页文件和写日志文件进行计时，并分析。
- 对比两个模型的结果，分析低效工作的IO操作。
- 寻求优化方案。
...
- 首先实现一个简易的压测工具，分析在同样压力、相同请求量下两个模型的差异。
...
- 分别针对多进程和多线程模型进行测试：`./webtest.py 16 1008`，获取结果。
- 分析两个模型的差异，并提出优化办法。
...
- 多线程模型测试结果：
  ...
  ```text
  Total requests:	1008
  read socket:	  0.16ms/time
  read web:	      0.00ms/time
  post data:	    1.30ms/time
  write log:	    0.10ms/time
  ```

  - 测试端输出：

  ```text
  http status:     {200: 1008}
  ms/time:         {200: 0.187}
  Total use time:  188.6810 ms
  ```
...
- 多进程魔性测试结果：
  ...
  Total requests:	1008
  read socket:	  0.35ms/time
  read web:	      0.00ms/time
  post data:	    1.20ms/time
  write log:	    0.13ms/time
  ```
...
- 结果分析：
  ...
  相对来看，两个模型在完成发送数据时都最耗时间，其次是读取socket和写日志相对耗时间。

- 优化方案：

  考虑降低锁的使用频率，在多线程模型高并发下，频繁的对锁进行操作是很耗时间的，同时也会造成其他线程的阻塞。

## 题目2

### 分析：

实际上，pthread的并发效率是有峰值的，在CPU核数*8以下，每个线程都能相对不被干扰的进行。而一旦超过了这个并发峰值，线程之间的干扰就会加剧，使得性能相对下降。不考虑网络速度的影响，pthread频繁开放线程会严重降低并发效率，但相对fork而言，线程的开销要小一些。
